...
const notAdminJwt = jwt.sign({ username: "test", isAdmin: false }, SECRET_KEY);
...
describe("PATCH /users/:username", () => {
  ...
  test("works: set new password for themselves", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        password: "new-password",
...
describe("DELETE /users/:username", function () {
  ...
  test("works for users to delete themselves", async function () {
    const resp = await request(app)
      .delete(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ deleted: "u1" });
